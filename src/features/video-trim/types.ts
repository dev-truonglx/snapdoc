...
/**
 * Vẽ một overlay đơn lẻ (rect, blur, text, arrow) trực tiếp lên canvas của video.
 */
export function drawOverlayItemOnCanvas(
  ...
  const vw = videoWidth > 0 ? videoWidth : 1280;
  const vh = videoHeight > 0 ? videoHeight : 720;
  const scale = Math.max(0.75, Math.min(2.5, vw / 1280));

  const rx = Math.round(vw * item.relX);
  const ry = Math.round(vh * item.relY);
  const rw = Math.max(16, Math.round(vw * item.relW));
  const rh = Math.max(16, Math.round(vh * item.relH));
  ...
  if (item.type === "blur") {
    if (item.isBlackout) {
      ...
      const sw = Math.min(vw - sx, rw + pad * 2);
      const sh = Math.min(vh - sy, rh + pad * 2);
      ...
      if (tempCtx) {
        ...
        ctx.clip();
        ctx.filter = `blur(${Math.max(8, Math.round(18 * scale))}px)`;
    ...
    const strokeWidth = Math.max(1, Math.round((item.strokeWidth || 3) * scale));
    ctx.save();
    ctx.strokeStyle = item.strokeColor || "#ef4444";
    ctx.lineWidth = strokeWidth;
    ...
    const lineHeight = fontSizePx * 1.35;
    const totalTextHeight = lines.length * lineHeight;
    let startY = (rh - totalTextHeight) / 2 + lineHeight / 2;
    ...
    const sx = rx + (item.arrowStartX ?? 0.1) * rw;
    const sy = ry + (item.arrowStartY ?? 0.1) * rh;
    const ex = rx + (item.arrowEndX ?? 0.9) * rw;
    const ey = ry + (item.arrowEndY ?? 0.9) * rh;
...
/**
 * Vẽ tất cả các overlay đang hiển thị tại mốc `playheadMs` (hoặc đang được chọn)
 * lên canvas trước khi xuất ảnh.
 */
export function drawOverlaysOnCanvas(
  ...
  for (const item of visibleOverlays) {
    drawOverlayItemOnCanvas(ctx, item, videoWidth, videoHeight, canvas);
  }
}
