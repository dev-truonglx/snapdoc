...
export type FocusZone =
  | "center"       // Giữa màn hình — phóng to đều đối xứng (translateX=0, translateY=0)
  | "left"         // Cột bên trái (Sidebar, Navigation)
  | "right"        // Cột bên phải (Panel, Inspector)
  | "top"          // Phía trên (Header, Menu, Tabs)
  | "bottom"       // Phía dưới (Dock, Terminal, Status Bar)
  | "top-left"     // Góc trên trái
  | "top-right"    // Góc trên phải
  | "bottom-left"  // Góc dưới trái
  | "bottom-right" // Góc dưới phải
  | "custom";      // Vị trí tuỳ chỉnh tự do

export interface ZoomSegment {
  id: string;
  startTimeMs: number;
  endTimeMs: number;
  scale: number; // e.g. 1.25, 1.5, 1.8, 2.0
  focusX: number; // 0.0 .. 1.0 (relative to video width)
  focusY: number; // 0.0 .. 1.0 (relative to video height)
  zone?: FocusZone;
  easing?: "smooth" | "snappy" | "instant";
  autoGenerated?: boolean;
}

export const DEFAULT_ZOOM_SCALE = 1.5;
export const ZOOM_SCALE_OPTIONS = [1.25, 1.5, 1.8, 2.0];
export const MIN_ZOOM_DURATION_MS = 600;

export function makeZoomSegmentUid(): string {
